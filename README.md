...
1. **Clone the Repository:**
   ```bash
   git clone https://github.com/Laadnanimoustapha/krili.git
   cd krili
   ```

2. **Open in Browser:**
   - Open `login-test1.html` to log in or `register-test1.html` to create a new account.
...
3. **PWA Installation:**
   - Open `rent-test1.html` in a supported browser.
   - The service worker (`sw.js`) will register automatically for offline support.

4. **Digital Contracts:**
   - During the rental workflow, contracts are shown in both French and Arabic for legal clarity.
